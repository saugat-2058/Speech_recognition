    ...
    def _accumulate_sufficient_statistics(
        ...
        if self._needs_sufficient_statistics_for_covars():
            if self.covariance_type in ("spherical", "diag"):
                ...
                # -> (nc, nf, nf)
                stats["obs*obs.T"] += np.einsum("ij,ik,il->jkl", posteriors, X, X)
    ...
    def _accumulate_sufficient_statistics(
        ...
        if "c" in self.params:  # covariance stats
            ...
            if self.covariance_type == "full":
                ...
                c_n = np.einsum("ijk,ijklm->jklm", post_comp_mix, centered_dots)
...
class BaseMultinomialHMM(BaseHMM):
    ...
    def _compute_likelihood(self, X):
        ...
        for c in range(self.n_components):
            probs[:, c] = multinomial.pmf(X, n=n_trials, p=self.emissionprob_[c, :])
        return probs
    ...
    def _generate_sample_from_state(self, state, random_state):
        ...
        return multinomial.rvs(
            n=n_trials, p=self.emissionprob_[state, :], random_state=random_state
        )
