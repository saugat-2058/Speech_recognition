...
def get_filterbank(numfilters, filterLen, lowFreq, highFreq, samplingFreq):
    ...
    f = (
        mel_to_freq(np.arange(numfilters + 2) * dwarp + minwarpfreq)
        * (filterLen - 1)
        * 2.0
        / samplingFreq
    )
    ...
    loslope = (f[2 : numfilters + 2] - i) / (
        f[2 : numfilters + 2] - f[1 : numfilters + 1]
    )
...
H = get_filterbank(
    numfilters=20, filterLen=257, lowFreq=0, highFreq=8000, samplingFreq=16000
)
...
class PreprocessedMFCCFeatures:
    ...
    def preprocess_audio(self, audio_file_path):
        ...
        delta_mfccs, delta_delta_mfccs = self.feature_extractor.compute_delta_mfcc(
            mfccs
        )
        ...
        scipy.io.wavfile.write(
            os.path.join(folder_name, f"paudio_file.wav"), sr, y_int16
        )
...
def preprocess_feature(file_path):
    ...
    mfccs, delta_mfccs, delta_delta_mfccs = preprocessed_mfcc_features.preprocess_audio(
        audio_file_path
    )
    ...
    concatenated_features = np.concatenate(
        (mfccs, delta_mfccs, delta_delta_mfccs), axis=0
    )
