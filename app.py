...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:@localhost/asr_users'
...
def signup_submit():
    ...
    password = bcrypt.hashpw(passwords.encode('utf-8'), bcrypt.gensalt())
    # print(username)
    # print(email)
    # print(password)
    ...
    if existing_user:
        return render_template('signup.html', message="User with the name already exists. Please try a different username.")
    else:
        existing_user = Users.query.filter_by(email=email).first()

        if existing_user:
            return render_template('signup.html', message="User with the name or email already exists. Please try a different username or email.")
        else:
            ...
            return render_template('login.html',message="signup Successfull now proceed to login")    
...
def login_submit():
    ...
    if existing_user:
            ...
            #   db.session.add(new_user)
              ...
              return render_template('indexs.html',message="Login Successgull",user=username)    
            else:
...
def test_sys():
    if 'user' in session:
        ...
        return render_template('login.html', message="Please Login In Order To Grant Access")
    ...
    if request.method == 'POST':
        ...
        if(password==confirm_password):
            ...
            if bcrypt.checkpw(password.encode('utf-8'), user.password.encode('utf-8')):
                return render_template("pass_change.html",message="current password and previous password mustnot be same")
            else:
                password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
                user.password = password
                db.session.commit()
                return render_template("profile.html",message="Password changed Successfully",user=user.username,email=user.email)
        else:
            return render_template("pass_change.html",message="confirm_password and password donot match")
    else: 
        if 'user' in session:
    ...
    if request.method == 'POST':
        ...
        for usersd in users:
            ...
            if usersd.email == email or usersd.username == username:
                return render_template("change_others.html",message='Username or email already exists')
            else:
                user.email = email
                user.username = username
                session['user'] = username
                db.session.commit()
                return render_template("profile.html",message="User Info Changed Successfully",user=user.username,email=user.email)
    else: 
        if 'user' in session:
...
def result():
    ...
    return render_template('result.html', image_paths=image_paths, json_paths=json_paths,audio_paths=audio_paths,)
